...
func TestScaleNumPredictForLocal(t *testing.T) {
	cases := map[int]int{
		0:    2000, // unset
		800:  2000, // inflater's default for cloud
		1500: 2000, // user bumped a bit but still below local floor
		2000: 2000,
		4000: 4000, // user explicitly asked for more — honor it
	}
	for in, want := range cases {
		if got := scaleNumPredictForLocal(in); got != want {
			t.Errorf("scaleNumPredictForLocal(%d) = %d, want %d", in, got, want)
		}
	}
}

func TestOllamaStreamSurfacesLengthTruncation(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/api/chat", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/x-ndjson")
		flusher, _ := w.(http.Flusher)
		...
		emit(`{"message":{"content":"Role: dev\nContext: working"},"done":false}`)
		emit(`{"message":{"content":""},"done":true,"done_reason":"length"}`)
	})
	srv := httptest.NewServer(mux)
	defer srv.Close()
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	ch, err := p.Stream(ctx, Request{System: "s", User: "u", MaxTokens: 800})
	if err != nil {
		t.Fatal(err)
	}
	var sb strings.Builder
	...
	if !strings.Contains(got, "cut off") {
		t.Errorf("expected truncation marker for done_reason=length, got: %q", got)
	}
}

func TestOllamaStreamCleanStopHasNoMarker(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/api/chat", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/x-ndjson")
		flusher, _ := w.(http.Flusher)
		...
		emit(`{"message":{"content":"hi"},"done":false}`)
		emit(`{"message":{"content":""},"done":true,"done_reason":"stop"}`)
	})
	srv := httptest.NewServer(mux)
	defer srv.Close()
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	ch, _ := p.Stream(ctx, Request{System: "s", User: "u", MaxTokens: 800})
	var sb strings.Builder
	for c := range ch {
		sb.WriteString(c)
	}
	if strings.Contains(sb.String(), "cut off") {
		t.Errorf("clean stop should NOT add truncation marker, got: %q", sb.String())
	}
}
